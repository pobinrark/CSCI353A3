...
def compute_spanning_tree(G):
	...
	att = nx.get_node_attributes(G, 'ports')
	ST.node[start]['ports'] = att[start]
	ST.node[start]['mactoport'] = {}
	while queue:
		vertex = queue.pop(0)
		for n in G.neighbors(vertex):
			print "look" + str(G.neighbors(vertex))
			if n not in visited:
				visited.add(n)
				ST.add_node(n) 
				ST.node[n]['ports'] = att[n]
				ST.node[n]['mactoport'] = {}
				queue.append(n)
				ST.add_edge(vertex, n)
	#ST = nx.minimum_spanning_tree(G)
...
def add_flow(datapath, port, dst, actions):
...
def flood_neighbors(dpid, datapath, msg, ST, dst):
	neighbors = get_neighbors(dpid, ST)
	actions = []
	for out_port in neighbors:
		actions.append(datapath.ofproto_parser.
...
class L2Forwarding(app_manager.RyuApp):
	...
	if dst in self.mac_to_port[dpid]:
		...
		add_flow(dp, msg.in_port, dst, actions)
	else:
		actions = flood_neighbors(dpid, dp, msg, self.ST, dst)
